...
const AddSchoolPage = ({ onSchoolAdded }) => {
  ...
  const addGradeField = () => {
    setFormData((prevData) => ({
      ...prevData,
      grades: [...prevData.grades, { name: '', sections: [''] }],
    }))
  }
        ...
        <div className="modal-content">
          ...
          <div className="modal-body">
            <form onSubmit={handleAddSchool}>
              <div className="mb-3">
                <label htmlFor="sch_name" className="form-label">
                  School Name
                </label>
                <input
                  type="text"
              ...
              <div className="mb-3">
                <label htmlFor="sch_code" className="form-label">
                  School Code
                </label>
                <input
                  type="text"
                  className="form-control"
                  id="sch_code"
                  value={formData.sch_code}
                  onChange={handleInputChange}
                  required
                />
              </div>
              <div className="mb-3">
                <label htmlFor="sch_est" className="form-label">
                  Established Date
                </label>
                <input
                  type="date"
                  className="form-control"
                  id="sch_est"
                  value={formData.sch_est}
                  onChange={handleInputChange}
                  required
                />
              </div>
              <div className="mb-3">
                <label htmlFor="sch_admin" className="form-label">
                  School Admin
                </label>
                <input
                  type="text"
              ...
              {formData.grades.map((grade, gradeIndex) => (
                <div key={gradeIndex}>
                  <div className="mb-3">
                    <label htmlFor={`grade-${gradeIndex}`} className="form-label">
                      Grade
                    </label>
                    <select
                  ...
                  {grade.sections.map((section, sectionIndex) => (
                    <div className="mb-3" key={sectionIndex}>
                      <label
                        htmlFor={`section-${gradeIndex}-${sectionIndex}`}
                      ...
                      <input
                        type="text"
                        className="form-control"
                        id={`section-${gradeIndex}-${sectionIndex}`}
                        value={section}
                        onChange={handleInputChange}
                        required
                      />
                  ...
                  <button
                    ...
                    onClick={() => addSectionField(gradeIndex)}
                  >
                    Add Section
                  </button>
                </div>
              ...
              <button type="button" className="btn btn-secondary mb-3" onClick={addGradeField}>
                Add Grade
              </button>
